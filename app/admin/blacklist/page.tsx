...
export default function BlacklistPage() {
  ...
  const getStatusBadge = (entry: BlacklistEntry) => {
    if (entry.is_permanent) {
      ...
      return <span className="px-2 py-1 text-white/60 rounded text-xs" style={{border: '1px solid rgba(93, 122, 156, 0.8)', backgroundColor: 'rgba(93, 122, 156, 0.8)'}}>已过期</span>;
    } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/60 text-sm mt-1">管理被封禁的玩家和IP地址</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl p-12 text-center" style={{border: '1px solid #2A3650', backgroundColor: 'rgba(30, 40, 60, 0.8)'}}>
          <p className="text-white/60 text-lg">暂无黑名单记录</p>
        </div>
      ) : (
        <>
          <div className="rounded-xl overflow-hidden" style={{backgroundColor: 'rgba(30, 40, 60, 0.8)'}}>
            <table className="w-full">
              <thead className="bg-transparent">
                <tr>
                  <th className="text-left px-4 py-3 text-white/60 font-medium">游戏ID</th>
                  <th className="text-left px-4 py-3 text-white/60 font-medium">IP地址</th>
                  <th className="text-left px-4 py-3 text-white/60 font-medium">封禁原因</th>
                  <th className="text-left px-4 py-3 text-white/60 font-medium">状态</th>
                  <th className="text-left px-4 py-3 text-white/60 font-medium">封禁人</th>
                  <th className="text-left px-4 py-3 text-white/60 font-medium">封禁时间</th>
              ...
              <tbody className="divide-y divide-white/20">
                {entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white/60 font-mono text-sm">{entry.ip_address || '-'}</td>
                    <td className="px-4 py-3 text-white/60 max-w-xs truncate" title={entry.reason}>
                      {entry.reason}
                    </td>
                    <td className="px-4 py-3">
                      {getStatusBadge(entry)}
                    </td>
                    <td className="px-4 py-3 text-white/60">{entry.banned_by}</td>
                    <td className="px-4 py-3 text-white/60 text-sm">{formatDate(entry.created_at)}</td>
                    <td className="px-4 py-3">
          ...
          {pagination.totalPages > 1 && (
            <div className="flex justify-center gap-2">
              <button
                ...
                className="px-4 py-2 hover:bg-white/30 disabled:opacity-50 text-white rounded-lg transition-all"
                style={{border: '1px solid rgba(93, 122, 156, 0.9)', backgroundColor: 'rgba(93, 122, 156, 0.9)'}}
              >
                上一页
              </button>
