...
export default function AnnouncementsPage() {
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      ...
      default:
        return <span className="px-2 py-1 text-white/60 rounded text-xs" style={{border: '1px solid rgba(93, 122, 156, 0.9)', backgroundColor: 'rgba(93, 122, 156, 0.3)'}}>普通</span>;
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-white/60 text-sm mt-1">发布和管理服务器公告</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl p-12 text-center" style={{border: '1px solid rgba(30, 40, 60, 0.7)', backgroundColor: 'rgba(30, 40, 60, 0.8)'}}>
          <p className="text-white/60 text-lg">暂无公告</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-transparent">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-white/60 font-medium">排序</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">标题</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">类型</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">创建人</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">创建时间</th>
                ...
                <tr key={announcement.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col items-center gap-1">
                      <button
                        ...
                        disabled={processing}
                        className="w-6 h-6 flex items-center justify-center text-white/60 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        ↑
                      </button>
                      <button
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {announcement.is_pinned && <span className="text-white/60">置顶</span>}
                      <span className="text-white font-medium">{announcement.title}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">{getTypeBadge(announcement.type)}</td>
                  <td className="px-4 py-3 text-white/60">{announcement.created_by || '系统'}</td>
                  <td className="px-4 py-3 text-white/60">{formatDate(announcement.created_at)}</td>
                  <td className="px-4 py-3">
      ...
      {showEditor && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-2xl p-6 w-full max-w-3xl shadow-2xl max-h-[90vh] overflow-y-auto" style={{border: '1px solid rgba(30, 40, 60, 0.7)', backgroundColor: 'rgba(30, 40, 60, 0.95)'}}>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-white text-sm mb-2 font-medium">标题 <span className="text-white/60">*</span></label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-white text-sm mb-2 font-medium">内容 <span className="text-white/60">*</span></label>
                <RichTextEditor
