    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-white/60 text-sm mt-1">添加、编辑和删除管理员账号，配置权限</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <th className="text-left px-4 py-3 text-white/60 font-medium">用户名</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">昵称</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">QQ</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">角色</th>
                <th className="text-left px-4 py-3 text-white/60 font-medium">操作</th>
              </tr>
            </thead>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        className="px-3 py-1.5 text-white rounded-lg transition-all text-sm"
                        style={{border: '1px solid rgba(93, 122, 156, 0.9)', backgroundColor: 'rgba(93, 122, 156, 0.9)'}}
                        onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(93, 122, 156, 1)'; }}
                        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'rgba(93, 122, 156, 0.9)'; }}
                      >
      ...
      {(showAddModal || editingAdmin) && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-black border border-white/20 rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-white text-sm mb-2 font-medium">用户名 <span className="text-white/60">*</span></label>
                <input
                  type="text"
                  value={formData.username}
                  onChange={(e) => setFormData({ ...formData, username: e.target.value })}
                  placeholder="登录用户名"
                  className="w-full bg-transparent border border-white/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-white/50"
                />
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="w-5 h-5 rounded bg-transparent border-white/20"
                    />
              ...
              {editingAdmin && editingAdmin.is_owner && (
                <div className="flex items-center gap-3 border-t border-white/20 pt-4">
                  <div className="w-5 h-5 rounded bg-transparent border border-white/20 flex items-center justify-center">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(permissionLabels).map(([key, label]) => (
                      <div key={key} className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor={`perm_${key}`} className="text-white/60 text-xs">{label}</label>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 px-4 py-3 bg-transparent border border-white/20 hover:bg-white/10 text-white rounded-xl transition-all"
              >
                取消
              </button>
              <button
                ...
                disabled={saving}
                className="flex-1 px-4 py-3 bg-white/20 hover:bg-white/30 text-white rounded-xl transition-all disabled:opacity-50"
              >
                {saving ? '保存中...' : '保存'}
              </button>
            </div>
      ...
      {showPermissionModal && permissionAdmin && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-black border border-white/20 rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <div className="space-y-5">
              {permissionCategories.map((category) => (
                <div key={category.name} className="bg-transparent rounded-xl p-4 border border-white/20">
                  <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
                    {category.name}
                  </h3>
                  <div className="grid grid-cols-1 gap-2">
                    {category.permissions.map((key) => (
                      <div key={key} className="flex items-center justify-between bg-transparent rounded-lg p-3 hover:bg-white/5 transition-colors border border-white/10">
                        <span className="text-white">{permissionLabels[key]}</span>
                        {permissionAdmin.is_owner ? (
                          <button
                            disabled
                            className="px-3 py-1.5 rounded-lg text-sm transition-all font-medium bg-transparent border border-white/50 text-white/60 cursor-not-allowed"
                          >
                             允许
                          </button>
                        ) : (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-sm transition-all font-medium ${
                              permissionAdmin.permissions[key]
                                ...
                                : 'bg-transparent border border-white/20 text-white/60 hover:bg-white/10'
                            }`}
                          >
              ...
              <div className="bg-transparent rounded-xl p-4 border border-white/20">
                <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
                   邮件通知设置
                </h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between bg-transparent rounded-lg p-3 hover:bg-white/5 transition-colors border border-white/10">
                    <div>
                  ...
                  <div className="flex items-center justify-between bg-transparent rounded-lg p-3 hover:bg-white/5 transition-colors border border-white/10">
                    ...
                    <button
                      disabled
                      className="px-3 py-1.5 rounded-lg text-sm transition-all font-medium bg-transparent border border-white/20 text-white/40 cursor-not-allowed"
                    >
                      未开放
                    </button>
                  </div>
                ...
                <div key={category.name} className="bg-gray-800/30 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {category.permissions.map((key) => {
                      ...
                      let buttonClass = 'bg-white/10 text-white border border-white/20 hover:bg-white/20';
                      let buttonText = '━ 不修改';
                      
                      if (state === true) {
                        buttonClass = 'bg-white/20 text-white border border-white/30 hover:bg-white/30';
                        buttonText = ' 允许';
                      } else if (state === false) {
                        buttonClass = 'bg-white/20 text-white border border-white/30 hover:bg-white/30';
                        buttonText = ' 禁止';
