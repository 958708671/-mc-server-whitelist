...
const client = new Client({
  connectionString: 'postgresql://neondb_owner:npg_Rs4dK7gknbSY@ep-autumn-frog-a15r2a2l-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require',
  ssl: {
...
async function addColumn(table, column, type, defaultValue = null) {
  ...
  if (defaultValue !== null) {
    query = `ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${column} ${type} DEFAULT ${defaultValue}`;
  } else {
  ...
  if (success) {
    ...
    console.log(`❌ 添加 ${table}.${column} 列失败`);
  }
}

async function createIndex(table, indexName, columns) {
  const query = `CREATE INDEX IF NOT EXISTS ${indexName} ON ${table}(${columns})`;
  const success = await executeQuery(query);
  if (success) {
    console.log(`✅ 成功创建索引 ${indexName} 于 ${table} 表`);
  } else {
    console.log(`❌ 创建索引 ${indexName} 失败`);
  }
}

async function updateNullValues(table, column, value) {
  const query = `UPDATE ${table} SET ${column} = ${value} WHERE ${column} IS NULL`;
  const success = await executeQuery(query);
  if (success) {
    console.log(`✅ 成功更新 ${table}.${column} 的 NULL 值`);
  } else {
    console.log(`❌ 更新 ${table}.${column} 失败`);
  }
}
